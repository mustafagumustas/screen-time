...
def main(fps=60, duration=0.35):
    ...
    df = DataHandler.DataHandler(
        excel_file="transformed_data.xlsx", number_of_frames=number_of_frames
    ).df

    canvas = Canvas.canvas()  # Set the width and height as needed
    ...
    # add static text
    static_text = canvas.canvas.create_text(
        720 / 2 + 150, 100, text="Most Followed Instagram Accounts", font=("Purisa", 30)
    )
    ...
    square = canvas.canvas.create_rectangle(0, 0, 1080, 1920)
